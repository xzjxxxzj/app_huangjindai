//
//  ChuangeMobileViewController.swift
//  huangjindai
//
...
class ChuangeMobileViewController: UIViewController, UITextFieldDelegate {
    ...
    fileprivate lazy var mobileText: UITextField = {
        ...
        let userMobile : String = (userDate["mobile"] as? String)!
        ...
        mobileText.textColor = UIColor(r: 204, g: 204, b: 204)
        //添加右按钮
    ...
    fileprivate lazy var loginBtn: UIButton = {
        let loginBtn = UIButton(frame : CGRect(x: kMobileW/4, y: textH*2 + 50, width: kMobileW/2, height: textH))
...
extension ChuangeMobileViewController {
    ...
    @objc fileprivate func clickBtn()
    {
        ...
        let params = ["userId" : userId, "mobileCode" : codeValue]
        model.oldMobileCheck(params: params, success: { (success) in
    ...
    fileprivate func setRetrun(_ type: Bool, _ msg: String, _ data:JSON = [])
    {
        if type {
            timeStop()
            let setNewMobileView = SetNewMobileViewController()
            self.navigationController?.pushViewController(setNewMobileView, animated: true)
        } else {
...
extension ChuangeMobileViewController {
    ...
    @objc fileprivate func sendCode() {
        ...
        //发送请求
        let userId = (userDate["userId"] as? String)!
        let params = ["userId" : userId, "type" : "3"]
