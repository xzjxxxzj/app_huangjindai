//
//  UserBackingInvestViewController.swift
//  huangjindai
//
...
class UserBackingInvestViewController: UIViewController {
...
extension UserBackingInvestViewController {
    ...
    fileprivate func loadData() {
        ...
        model.getInvestList(params: ["userId" : userId,"status" : 8,"page" : 1], success: { (retrunData) in
    ...
    fileprivate func loadDataPage() {
        let userId = userData["userId"] ?? ""
        model.getInvestList(params: ["userId" : userId,"status" : 8,"page" : page], success: { (retrunData) in
            if retrunData["code"].intValue == 1 {
    ...
    fileprivate func setData(_ returnData : JSON) {
        ...
        for (index, investData) : (String, JSON) in returnData["list"] {
            ...
            view.endMoney.text = "\(investData["sum"].doubleValue)"
            //开启交互
