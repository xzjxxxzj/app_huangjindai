//
//  UserEndInvestViewController.swift
//  huangjindai
//
...
class UserEndInvestViewController: UIViewController {
...
extension UserEndInvestViewController {
    ...
    fileprivate func loadData() {
        ...
        model.getInvestList(params: ["userId" : userId,"status" : 9,"page" : 1], success: { (retrunData) in
    ...
    fileprivate func loadDataPage() {
        let userId = userData["userId"] ?? ""
        model.getInvestList(params: ["userId" : userId,"status" : 9,"page" : page], success: { (retrunData) in
            if retrunData["code"].intValue == 1 {
    ...
    fileprivate func setData(_ returnData : JSON) {
        ...
        for (index, investData) : (String, JSON) in returnData["list"] {
            ...
            view.centerName.text = "已回款（ 元 ）"
            view.centerMoney.text = "\(investData["getSum"].doubleValue)"
            view.endName.text = "融资期限( 月 )"
            view.endMoney.text = "\(investData["dealLimit"].doubleValue)"
            //开启交互
