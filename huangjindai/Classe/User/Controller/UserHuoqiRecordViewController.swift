//
//  UserHuoqiRecordViewController.swift
//  huangjindai
//
...
class UserHuoqiRecordViewController: UIViewController {
...
extension UserHuoqiRecordViewController {
    ...
    fileprivate func loadData() {
        ...
        model.huoqiRecord(params: ["userId" : userId,"page" : 1], success: { (retrunData) in
    ...
    fileprivate func loadDataPage() {
        let userId = userData["userId"] ?? ""
        model.huoqiRecord(params: ["userId" : userId,"page" : page], success: { (retrunData) in
            if retrunData["code"].intValue == 1 {
    ...
    fileprivate func setData(_ returnData : JSON) {
        ...
        for (index, listData) : (String, JSON) in returnData["list"] {
            ...
            scollH = Y
            let listView = UIView(frame: CGRect(x: 0, y: Y, width: kMobileW, height: listH))
            if listAll % 2 != 0 {
            ...
            var x: CGFloat = 10
            for tital in [listData["time"].stringValue,"\(listData["money"].stringValue)",listData["type"].stringValue] {
                let label = UILabel(frame : CGRect(x: x, y: 0, width: (kMobileW - 10)/3, height: listH))
                x = x + kMobileW/3
                label.text = tital
                label.textAlignment = NSTextAlignment.center
                label.font = UIFont.systemFont(ofSize: 14)
            ...
            recordListView.append(listView)
            contenView.addSubview(listView)
        }
