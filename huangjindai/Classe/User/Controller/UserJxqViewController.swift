...
import SwiftyJSON
//列表高度
fileprivate let jxqH: CGFloat = 110
fileprivate let jxqHearH : CGFloat = 40
...
class UserJxqViewController: UIViewController {
    ...
    fileprivate lazy var heardView : DjqHearView = {
        ...
        heardView.frame = CGRect(x: 0, y: 1, width: kMobileW, height: jxqHearH)
        return heardView
    }()
    
    fileprivate lazy var willView : UIView = {
        let willView = UIView(frame: CGRect(x: 0, y: jxqHearH, width: kMobileW, height: 20))
...
extension UserJxqViewController {
    ...
    fileprivate func loadData() {
        ...
        model.getDjqList(params: ["userId" : userId,"cardType" : 3,"page" : 1], success: { (retrunData) in
    ...
    fileprivate func loadDataPage() {
        let userId = userData["userId"] ?? ""
        model.getDjqList(params: ["userId" : userId,"cardType" : 3,"page" : page], success: { (retrunData) in
            if retrunData["code"].intValue == 1 {
    ...
    fileprivate func setData(_ returnData : JSON) {
        let keyong = "可用：\(returnData["info"]["canUse"].stringValue)张"
        ...
        let useTex = "已用：\(returnData["info"]["use"].stringValue)张"
        ...
        if returnData["info"]["willExper"].intValue > 0 {
            ...
            let willTxt = "您有\(returnData["info"]["willExper"].stringValue)张加息券即将在\(returnData["info"]["time"])天内到期，请尽快使用！"
            ...
            view.addSubview(willView)
            collectionView.frame = CGRect(x: 0, y: jxqHearH + 20, width: kMobileW, height: collectH - jxqHearH - 20)
        }else {
            willView.removeFromSuperview()
            collectionView.frame = CGRect(x: 0, y: jxqHearH + 1, width: kMobileW, height: collectH - jxqHearH - 1)
        }
        page += 1
        for (index, jxqData) : (String, JSON) in returnData["list"] {
            ...
            scollH = Y
            view.frame = CGRect(x: 10, y: Y, width: kMobileW - 20, height: jxqH)
        ...
        self.collectionView.contentSize = CGSize(width : kMobileW, height: scollH + jxqH + kHotH)
    }
