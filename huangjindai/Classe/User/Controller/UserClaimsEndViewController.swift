//
//  UserClaimsEndViewController.swift
//  huangjindai
//
...
class UserClaimsEndViewController: UIViewController {
...
extension UserClaimsEndViewController {
    ...
    fileprivate func loadData() {
        ...
        model.getClaimsIngList(params: ["userId" : userId,"status" : 2,"page" : 1], success: { (retrunData) in
    ...
    fileprivate func loadDataPage() {
        let userId = userData["userId"] ?? ""
        model.getClaimsIngList(params: ["userId" : userId,"status" : 2,"page" : page], success: { (retrunData) in
            if retrunData["code"].intValue == 1 {
    ...
    @objc fileprivate func viewClick(_ sender: UITapGestureRecognizer)
    {
        ...
        let detailView = UserClaimsEndDetailViewController()
