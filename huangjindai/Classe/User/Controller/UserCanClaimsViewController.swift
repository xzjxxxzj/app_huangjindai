//
//  UserCanClaimsViewController.swift
//  huangjindai
//
...
class UserCanClaimsViewController: UIViewController {
...
extension UserCanClaimsViewController {
    ...
    fileprivate func loadData() {
        ...
        model.getCanClaimsList(params: ["userId" : userId,"page" : 1], success: { (retrunData) in
    ...
    fileprivate func loadDataPage() {
        let userId = userData["userId"] ?? ""
        model.getCanClaimsList(params: ["userId" : userId,"page" : page], success: { (retrunData) in
            if retrunData["code"].intValue == 1 {
    ...
    fileprivate func setData(_ returnData : JSON) {
        ...
        for (index, investData) : (String, JSON) in returnData["list"] {
            ...
            view.firstName.text = "待收本息( 元 )"
            view.firstMoney.text = "\(investData["nopayMoney"].doubleValue)"
            view.centerName.text = "年化收益率"
            view.centerMoney.text = "\(investData["rate"].stringValue)"
            view.endName.text = "下次回款"
            view.endMoney.text = investData["nextRepayTime"].stringValue
            //开启交互
    ...
    @objc fileprivate func viewClick(_ sender: UITapGestureRecognizer)
    {
        ...
        let detailView = UserClaimsViewController()
